...
*/
const MONTHLY_API_CALL = async (symbol, outputSize, month) => {
	/*
    Or, we can do
	...
	*/
	...
	for (var key in json["Time Series (Daily)"]) {
		...
		if (date.getTime() >= MONTH_AGO_DATE.getTime()) {
			const obj = {
				date: key,
...
module.exports = { MONTHLY_API_CALL };
